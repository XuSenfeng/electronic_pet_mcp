...
public:
    ...
    bool Connect(const std::string broker_address, int broker_port, const std::string client_id, const std::string username, const std::string password); // 建立连接
    void Disconnect(); // 关闭连接
    bool Publish(const std::string topic, const std::string payload, int qos = 0); // 发布数据
    bool Subscribe(const std::string topic, int qos = 0); // 订阅消息
    bool Unsubscribe(const std::string topic); // 取消订阅
    bool IsConnected(); .. 数据检查
