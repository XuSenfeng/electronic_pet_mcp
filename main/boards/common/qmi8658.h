...
public:
    ...
    void deal_qmi8658_data(t_sQMI8658 *p)
    {
        ...
        if(X_tmp >= 50 && app.position_l == false){
            // ↓
            ESP_LOGI("QMI8658", "gesture event down");
        ...
        if(X_tmp <= -50 && app.position_r == false){
            // ↑
            ESP_LOGI("QMI8658", "gesture event up");
        ...
        if(Y_tmp >= 50 && app.position_f == false){
            // →
            ESP_LOGI("QMI8658", "gesture event right");
            app.position_l = true;
            if(display->screen_now_ == display->screen_state_) {
                ...
                int game = pet->GetCurrentGame();
                game+=1;
                if(game >= pet->games_.size()){
                    game = 0;
                }
        ...
        if(Y_tmp <= -50 && app.position_b == false){
            // ←
            ESP_LOGI("QMI8658", "gesture event left");
            app.position_r = true;
            if (display->screen_now_ == display->screen_main_) {
