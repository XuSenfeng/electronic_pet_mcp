 ...
 * @LastEditTime: 2025-09-07 23:07:05
 * @FilePath: /xiaozhi-esp32/main/display/lcd_display_specification.cc
 */
...
const char * help_content[][2] = {
    ...
    {"核心玩法", "• 喂食  保持饱食度\n"
         ...
         "• 商店  购买道具物品\n"
         "• 升级  提升宠物的属性\n"},
    {"状态说明", "经验值：在经验值达到一定阈值以后可以升级, 升级会提升宠物的状态, 并解锁物品\n"
         "心情值：互动频率影响\n"
         "饱食度：随时间自然消耗, 为0的时候会寄寄\n"
         "金钱：打工获取用于购买\n"
         "IQ：可以提升工作的工资"},
#if CONFIG_BOARD_TYPE_XVSENFAI
    {"操作指南", "左侧按键, 控制小智的各个状态切换\n"
         "中间按键, 不同菜单之间的切换, 长按进行小菜单之间的切换\n"
         "右侧按键, 返回按键, 返回主界面或者上层界面"},
#elif CONFIG_BOARD_TYPE_LICHUANG_DEV
    {"操作指南", "← 状态界面\n"
         "→ 商店界面\n"
         "↓ 说明界面\n"
         "↑ 设置界面"},
 ...
 void LcdDisplay::HelpUI() {
    ...
    CreateCard(scroll_cont, 0, LV_SYMBOL_PLAY, boardID.c_str(), help_content[0][1]);

    for(int i = 1; i < sizeof(help_content) / sizeof(help_content[0]); i++){
        CreateCard(scroll_cont, i, LV_SYMBOL_HOME, help_content[i][0], help_content[i][1]);
    }
}
...
{
    if(cards == nullptr){
        cards = new HelpCard[sizeof(help_content) / sizeof(help_content[0])];
    }
    // 可爱风格卡片容器
